...
export default function ApplyPage() {
  ...
  useEffect(() => {
    if (session?.user?.name) setValue("fullName", session.user.name, { shouldValidate: true });
  ...
  const basicFields = watch(["phone", "address", "district", "pinCode", "aadharNumber", "dateOfBirth"]);
  useEffect(() => {
    if (isDraftSaved) {
  ...
  const onSaveDraft = async (data: ApplicationFormType) => {
    try {
      ...
      if (response.data?.success) {
        ...
        setApplicationId(response.data.applicationId || response.data.data?._id || null);
  ...
  const handleProceedToPayment = () => {
    if (!isDraftSaved || !applicationId) return;
      ...
      <div className="w-full max-w-4xl bg-card border rounded-[32px] shadow-sm overflow-hidden my-4">
        ...
        <div className="border-b px-6 md:px-16 py-10 md:py-12">
          ...
          <p className="text-muted-foreground mt-3 text-[15px] leading-7">
            Fill in all details carefully, upload files, and save your draft to unlock checkout payments.
          </p>
        </div>

        {/* FORM */}
        <form onSubmit={handleSubmit(onSaveDraft)} className="p-6 md:p-16 space-y-16">
          ...
          <section className="space-y-12">
            <div className="space-y-3">
              ...
              <p className="text-muted-foreground text-sm">Enter your personal and address details.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              <div className="space-y-4">
                <Label className="text-sm font-medium pl-1">Full Name</Label>
                <Input disabled value={session?.user?.name || ""} className="h-13 px-5 text-[15px] rounded-2xl bg-muted text-muted-foreground" />
              ...
              <div className="space-y-4">
                ...
                <Input disabled value={session?.user?.email || ""} className="h-13 px-5 text-[15px] rounded-2xl bg-muted text-muted-foreground" />
                {errors.email && <p className="text-sm text-red-500 pl-1">{errors.email.message}</p>}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              <div className="space-y-4">
                ...
                <Input {...register("phone")} placeholder="Enter phone number" className="h-13 px-5 text-[15px] rounded-2xl" />
                {errors.phone && <p className="text-sm text-red-500 pl-1">{errors.phone.message}</p>}
              </div>
              ...
              <div className="space-y-4 relative z-50">
                ...
                {errors.gender && <p className="text-sm text-red-500 pl-1">{errors.gender.message}</p>}
              </div>
            </div>

            <div className="space-y-4">
              <Label className="text-sm font-medium pl-1">Address</Label>
              <Textarea {...register("address")} placeholder="Enter complete address" className="min-h-35 rounded-2xl px-5 py-4 text-[15px] leading-relaxed" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              <div className="space-y-4">
                ...
                <Input {...register("district")} placeholder="Enter district" className="h-13 px-5 text-[15px] rounded-2xl" />
                {errors.district && <p className="text-sm text-red-500 pl-1">{errors.district.message}</p>}
              </div>
              <div className="space-y-4">
                <Label className="text-sm font-medium pl-1">PIN Code</Label>
                <Input {...register("pinCode")} placeholder="Enter PIN code" className="h-13 px-5 text-[15px] rounded-2xl" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              <div className="space-y-4">
                <Label className="text-sm font-medium pl-1">Date of Birth</Label>
                <Input type="date" {...register("dateOfBirth")} className="h-13 px-5 text-[15px] rounded-2xl" />
                {errors.dateOfBirth && <p className="text-sm text-red-500 pl-1">{errors.dateOfBirth.message}</p>}
              </div>
              <div className="space-y-4">
                <Label className="text-sm font-medium pl-1">Aadhaar Number</Label>
                <Input {...register("aadharNumber")} placeholder="Enter Aadhaar number" className="h-13 px-5 text-[15px] rounded-2xl" />
                {errors.aadharNumber && <p className="text-sm text-red-500 pl-1">{errors.aadharNumber.message}</p>}
              </div>
            </div>
          </section>
          ...
          <section className="space-y-12 border-t pt-14">
            <div className="space-y-3">
              <h2 className="text-2xl font-semibold text-card-foreground">Upload Documents</h2>
              <p className="text-muted-foreground text-sm">Upload files directly to secure cloud storage.</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* PHOTO */}
              <div className={`rounded-3xl border p-8 space-y-8 flex flex-col justify-between ${photoUrl ? 'border-green-500 bg-green-50/10' : 'bg-muted/20'}`}>
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-xl shrink-0 ${photoUrl ? 'bg-green-100 text-green-600' : 'bg-primary/10 text-primary'}`}>
                    {photoUrl ? <CheckCircle2 size={20} /> : <UploadCloud size={20} />}
                  </div>
                  <div>
                    <h3 className="font-semibold text-base">Passport Photo</h3>
                    <p className="text-xs text-muted-foreground mt-1.5">JPG or PNG</p>
                  </div>
                </div>
                <div>
                  <Input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        const url = await uploadFile(file, "applications/photos");
                  ...
                  />
                  {errors.photoUrl && <p className="text-xs text-red-500 mt-2 pl-1">{errors.photoUrl.message}</p>}
                </div>
              </div>

              {/* SIGNATURE */}
              <div className={`rounded-3xl border p-8 space-y-8 flex flex-col justify-between ${signatureUrl ? 'border-green-500 bg-green-50/10' : 'bg-muted/20'}`}>
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-xl shrink-0 ${signatureUrl ? 'bg-green-100 text-green-600' : 'bg-primary/10 text-primary'}`}>
                    {signatureUrl ? <CheckCircle2 size={20} /> : <UploadCloud size={20} />}
                  </div>
                  <div>
                    <h3 className="font-semibold text-base">Signature</h3>
                    <p className="text-xs text-muted-foreground mt-1.5">Clear image file</p>
                  </div>
                </div>
                <div>
                  <Input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        const url = await uploadFile(file, "applications/signatures");
                    ...
                    className="h-12 rounded-xl cursor-pointer"
                  />
                  {errors.signatureUrl && <p className="text-xs text-red-500 mt-2 pl-1">{errors.signatureUrl.message}</p>}
                </div>
              </div>

              {/* AADHAAR */}
              <div className={`rounded-3xl border p-8 space-y-8 flex flex-col justify-between ${aadharDocumentUrl ? 'border-green-500 bg-green-50/10' : 'bg-muted/20'}`}>
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-xl shrink-0 ${aadharDocumentUrl ? 'bg-green-100 text-green-600' : 'bg-primary/10 text-primary'}`}>
                    {aadharDocumentUrl ? <CheckCircle2 size={20} /> : <UploadCloud size={20} />}
                  </div>
                  <div>
                    <h3 className="font-semibold text-base">Aadhaar Document</h3>
                    <p className="text-xs text-muted-foreground mt-1.5">PDF or image</p>
                  </div>
                </div>
                <div>
                  <Input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        const url = await uploadFile(file, "applications/aadhar");
                        setValue("aadharDocumentUrl", url, { shouldValidate: true });
                      } catch (error) {
                    ...
                    className="h-12 rounded-xl cursor-pointer"
                  />
                  {errors.aadharDocumentUrl && <p className="text-xs text-red-500 mt-2 pl-1">{errors.aadharDocumentUrl.message}</p>}
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="border-t pt-12 flex flex-col md:flex-row gap-8 md:items-center md:justify-between">
            ...
            <div className="flex flex-col sm:flex-row gap-4 shrink-0">
              ...
              <Button
                type="submit"
                disabled={loading || uploadingPhoto || uploadingSignature || uploadingAadhar}
              ...
              >
                {loading && isDraftSaved ? (
                  <span className="flex items-center gap-2"><Loader2 className="animate-spin" size={18} />Redirecting...</span>
                ) : (
